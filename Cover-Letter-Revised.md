Josh Hamrick
Programmer, New Course Communications
November 2025

To the new leadership team,

I'm Josh, a software engineer and programmer at New Course. I'm excited by your focus on improving our software, workflow efficiency, and profitability. Over the past several months, I've mapped the biggest friction points our programmers face and built a prototype that automates letter template formatting. I'm glad to share a short demo and sample document of the latest iteration whenever it's useful.

I believe we can transform programmer efficiency through two massive checkpoints:

**Phase 1: ~50% Efficiency Gain** — Foundational automation that cuts manual work in half through initiatives like a dedicated in-house QC team, automated letter template formatting, removing high-friction steps in NcConnect, and streamlining CRF/file handling.

**Phase 2: 95% Faster** — Once Phase 1 is in place, we can build end-to-end automation that transforms CRF creation → "Samples Under Review" from hours/days to almost instantaneous.

I've attached a detailed proposal outlining this vision, with the Letter Template Formatter as the lead Phase 1 initiative that delivers production-ready HTML aligned to our programming standards. The architecture of all Phase 1 initiatives is designed to enable the ultimate Phase 2 automation pipeline.

I'll be honest with you: I don't feel like my expertise is being used to its full potential in my current position. I see opportunities to improve our processes and profitability here at New Course, but I've felt unable to speak openly about this to management. I'm reaching out to you because I feel we share a similar mindset around growth and innovation. This company moves in ways that feel archaic at times, and I've seen solution after solution with no opportunity to implement them.

I'm passionate about improving this company through software solutions, and I believe I could be a real asset if given the chance. With my AI expertise—which I consider one of my strongest skills—I can accelerate the production of these solutions significantly. I operate at a high level and can deliver products that might take a team months in weeks. I'm not saying this to boast, but to be transparent about what I believe I can contribute when given the right opportunity.

I'm eager to build my career as a developer, whether by driving the automation that strengthens our software and day-to-day workflows or serving as a core member of the software team. I'm eager to contribute wherever my skills can have the greatest impact on the company's success.

I'd be happy to work in any position you need, especially as you're implementing new technologies. I may be more useful getting things rolling there instead of automating the workflow initially. So, humbly, I come to you and ask that you consider me for any position you see fit. I'm ready to help us move faster and make it easier for our clients and teams to succeed.

Thank you for your time and leadership. I'm looking forward to the possibility of working together.

Josh

Attachment: Programmer Efficiency & Profitability Initiatives (Markdown)

