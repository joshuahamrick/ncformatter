...
class WordFormatter {
    ...
    _clientSidePIICheck(ir) {
        ...
        // Descriptive bracket placeholders such as [BorrowerName] or [Company Long Name].
        // Weak on their own, so several distinct ones are required (mirrors pii_scanner.py).
        ...
        if (!hasTemplateVars) {
            const distinct = new Set(totalText.match(genericPlaceholderPattern) || []);
            if (distinct.size >= GENERIC_PLACEHOLDER_MIN_DISTINCT) hasTemplateVars = true;
        }
