...
class DocumentProcessor {
            ...
            reader.onload = function(event) {
                ...
                mammoth.convertToHtml({arrayBuffer: arrayBuffer})
                    .then(function(result) {
                        ...
                        // Clean up the extracted HTML text
                        ...
                        console.log('Extracted HTML text from Word document:', extractedText.substring(0, 200) + '...');
    ...
    static applyCustomFormatting(text, rules) {
        // Apply universal formatting rules to ANY document - NO DOCUMENT-SPECIFIC RULES
        console.log('Applying universal formatting rules to ANY document');
        
        let formattedText = text;
        
        // Apply universal rules that work for ALL documents - NO EXCEPTIONS
    ...
    static detectDocumentType(text) {
        if (text.includes('Notice of Breach') && text.includes('default under the Note and Mortgage')) {
            ...
            return 'BR010';
        } else if (text.includes('Privacy Policy') || text.includes('FACTS') || text.includes('Federal Privacy Model Form') || text.includes('personal information')) {
            return 'PRIVACY';
        } else if (text.includes('L001E8') && text.includes('maturity date') && text.includes('payoff statement')) {
            return 'SL106';
        }
        return 'Generic';
    }
    ...
    static formatPrivacyDocument(text) {
        // Format Federal Privacy Model Form documents
        console.log('Formatting Privacy Policy document...');
        
        // Generate the exact HTML output for privacy policy
        return this.generatePrivacyPolicyHTML();
    }
    
    static generatePrivacyPolicyHTML() {
        // Return the complete, properly formatted privacy policy HTML with ALL content
        return `{Font(Arial|9pt)}
<b><div style="text-align: center; font-size: 14pt">Privacy Policy</div></b>
<div style="text-align: right; font-size: 8pt">Rev 06.01.2024</div>
<table width="100%" style="border-collapse: collapse"><tbody><tr>
  ...
  <td valign="top" style="padding: 10px; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(255, 255, 255, 1); font-size: 14pt"><b style="font-size: 14pt; color: rgba(255, 255, 255, 1)">Why?</b></td> 
...
<table width="100%" style="border-collapse: collapse"><tbody><tr>
  ...
  <td style="padding: 10px 0 0 10px; border: 1px solid rgba(188, 190, 192, 1)"><div style="padding-bottom: 10px">Companies related by common ownership or control. They can be financial and nonfinancial companies.</div>
    <table width="100%"><tbody><tr>
      <td width="10%" valign="top">{Symbol(n)}</td> 
      <td><i>We do not have affiliates at this time.</i></td>
      </tr></tbody></table>
    <br>
    </td>
  </tr><tr>
  ...
  <td style="padding: 10px 0 0 10px; border: 1px solid rgba(188, 190, 192, 1)"><div style="padding-bottom: 10px">Companies not related by common ownership or control. They can be financial and nonfinancial companies.</div>
    <table width="100%"><tbody><tr>
      <td width="10%" valign="top">{Symbol(n)}</td> 
      <td><i>We do not share with nonaffiliates at this time.</i></td>
      </tr></tbody></table>
    <br>
    </td>
  </tr><tr>
  ...
  <td style="padding: 10px 0 0 10px; border: 1px solid rgba(188, 190, 192, 1)"><div style="padding-bottom: 10px">A formal agreement between nonaffiliated financial companies that together market financial products or services to you.</div>
    <table width="100%"><tbody><tr>
      <td width="10%" valign="top">{Symbol(n)}</td> 
      <td><i>We do not share with joint marketing partners at this time.</i></td>
      </tr></tbody></table>
    <br>
    </td>
  </tr></tbody></table>
...
<table width="100%" style="border-collapse: collapse"><tbody><tr>
  ...
  <td style="border: 1px solid rgba(188, 190, 192, 1)"><br></td>
    ...
    static formatSL106Document(text) {
        // Format SL106 maturity date documents with proper header structure
        let formatted = text;
        
        // 1. Fix field names - convert L001E8 to L001
        formatted = formatted.replace(/#L001E8#/g, '{[L001]}');
        
        // 2. Fix field names - convert M558, M559, etc. to proper format
        formatted = formatted.replace(/#M558#/g, '{[M558]}');
        formatted = formatted.replace(/#M559#/g, '{[M559]}');
        formatted = formatted.replace(/#M560#/g, '{[M560]}');
        formatted = formatted.replace(/#M561#/g, '{[M561]}');
        formatted = formatted.replace(/#M562#/g, '{[M562]}');
        formatted = formatted.replace(/#M563#/g, '{[M563]}');
        formatted = formatted.replace(/#M564#/g, '{[M564]}');
        formatted = formatted.replace(/#M565#/g, '{[M565]}');
        formatted = formatted.replace(/#M566#/g, '{[M566]}');
        formatted = formatted.replace(/#M594#/g, '{[M594]}');
        formatted = formatted.replace(/#M567#/g, '{[M567]}');
        formatted = formatted.replace(/#M568#/g, '{[M568]}');
        formatted = formatted.replace(/#M583#/g, '{[M583]}');
        formatted = formatted.replace(/#U072#/g, '{[U072]}');
        formatted = formatted.replace(/#U032#/g, '{[U032]}');
        formatted = formatted.replace(/#U234E4#/g, '{[U234E4]}');
        formatted = formatted.replace(/#L003#/g, '{[L003]}');
        formatted = formatted.replace(/#L004#/g, '{[L004]}');
        formatted = formatted.replace(/#L005#/g, '{[L005]}');
        
        // 3. Fix company field references
        ...
        formatted = formatted.replace(/<LoanCounselingPh>/g, '{[plsMatrix.LoanCounselingPh]}');
        ...
        formatted = formatted.replace(/<CashMgmtDept>/g, '{[plsMatrix.CashMgmtDept]}');
        formatted = formatted.replace(/<LossMitHrs>/g, '{[plsMatrix.LossMitHrs]}');
        
        // 4. Create proper header structure - replace the entire messy header section
        formatted = formatted.replace(
            /<div>\{\[L001\]\}<\/div>\s*<br>\s*<div>\{\[M558\]\}<\/div>\s*<br>\s*<div>\{\[M559\]\}<\/div>\s*<br>\s*<div>\{\[M560\]\}<\/div>\s*<br>\s*<div>\{\[M561\]\}<\/div>\s*<br>\s*<div>\{\[M562\]\}<\/div>\s*<br>\s*<div>\{\[M563\]\}, \{\[M564\]\} \{\[M565\]\} \{\[M566\]\}<\/div>/g,
            '<div>{[tagHeader]}</div><br><div style="text-align: right">{[L001]}</div><br><div>{[mailingAddress]}</div>'
        );
        
        // 5. Create RE: table structure - replace the loan/property section
        formatted = formatted.replace(
            /<div>Re: Loan No: \{\[M594\]\}<\/div>\s*<br>\s*<div>Property Address: \{\[M567\]\}<\/div>\s*<br>\s*<div>\{\[M568\]\}<\/div>/g,
            '<div><table width="100%" style="border-collapse: collapse"><tbody><tr>\n  <td width="20%">RE: Loan No:</td>\n  <td>{[M594]}</td>\n  </tr><tr>\n  <td width="20%" valign="top">Property Address:</td>\n  <td>{Compress({[M567]}|{[M583]}|{[M568]})}</td>\n</tr></tbody></table></div>'
        );
        
        // 6. Handle translation assistance paragraphs - keep them separate with center alignment
        formatted = formatted.replace(
            /<div>For homeowners who require any translation assistance or Language Access Services, please contact us at \{\[plsMatrix\.CSPhoneNumber\]\}\. A translation of this letter into a language other than English may be obtained\.<\/div>\s*<br>\s*<div>Si necesita asistencia con la traduccion o servicios de acceso al idioma, llamenos al \{\[plsMatrix\.CSPhoneNumber\]\}\.<\/div>\s*<br>\s*<div>Se puede obtener una traduccion de esta carta\.<\/div>/g,
            '<div style="text-align: center">For homeowners who require any translation assistance or Language Access Services, please contact us at {[plsMatrix.CSPhoneNumber]}. A translation of this letter into a language other than English may be obtained.</div><div style="text-align: center">Si necesita asistencia con la traduccion o servicios de acceso al idioma, llamenos al {[plsMatrix.CSPhoneNumber]}.</div><div style="text-align: center">Se puede obtener una traduccion de esta carta.</div>'
        );
        
        // 7. Add missing salutation after translation paragraphs
        formatted = formatted.replace(
            /<div style="text-align: center">Se puede obtener una traduccion de esta carta\.<\/div>\s*<br>\s*<br>\s*<div>This notice is to inform you/g,
            '<div style="text-align: center">Se puede obtener una traduccion de esta carta.</div><br><br><div>Dear Mortgager(s),</div><br><br><div>This notice is to inform you'
        );
        
        // 8. Fix money formatting
        formatted = formatted.replace(/\$\{\[U234E4\]\}/g, '{Money({[U234E4]})}');
        
        return formatted;
    }
    
    static formatPrivacyHeaders(text) {
        let formatted = text;
        
        // 1. Format main privacy policy header
        formatted = formatted.replace(
            /Privacy Policy/gi,
            '<b><div style="text-align: center; font-size: 14pt">Privacy Policy</div></b>'
        );
        
        // 2. Format revision date (right-aligned, 8pt) - handle various date formats
        formatted = formatted.replace(
            /Rev\s*(\d{2}\.\d{2}\.\d{4})/gi,
            '<div style="text-align: right; font-size: 8pt">Rev $1</div>'
        );
        
        // 3. Create proper FACTS table structure
        formatted = formatted.replace(
            /FACTS\s*WHAT DOES\s*<([^>]+)>\s*DO WITH YOUR\s*PERSONAL INFORMATION\?/gi,
            '<table width="100%" style="border-collapse: collapse"><tbody><tr>\n  <td valign="top" width="17%" style="padding: 10px; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(0, 0, 0, 1); font-size: 20pt"><b style="color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); font-size: 20pt">FACTS</b></td>\n  <td style="padding: 10px; border-bottom: 1px solid rgba(0, 0, 0, 1); font-size: 11.5pt"><b style="font-size: 11.5pt">WHAT DOES {Upper({[plsMatrix.CompanyLongName]})} DO WITH YOUR<br>PERSONAL INFORMATION?</b></td>\n  </tr><tr>\n  <td></td>\n  <td></td>\n  </tr></tbody></table>'
        );
    ...
    static formatPrivacyTables(text) {
        let formatted = text;
        
        // 1. Fix field references to proper plsMatrix format
        formatted = formatted.replace(/<CompanyLongName>/g, '{[plsMatrix.CompanyLongName]}');
        ...
        // 2. Format Why/What/How sections
        formatted = this.formatPrivacySection(formatted, 'Why\\?', 'Financial companies choose how they share your personal information. Federal law gives consumers the right to limit some but not all sharing. Federal law also requires us to tell you how we collect, share, and protect your personal information. Please read this notice carefully to understand what we do.');
        
        formatted = this.formatPrivacySection(formatted, 'What\\?', 'The types of personal information we collect and share depend on the product or service you have with us. This information can include:<br> \n    <table width="100%"><tbody><tr>\n      <td width="05%" valign="top" style="text-align: center">{Symbol(n)}</td> \n      <td>Social Security number, income and employment information</td>\n      </tr><tr>\n      <td valign="top" style="text-align: center">{Symbol(n)}</td> \n      <td>account balances and payment history</td> \n      </tr><tr>\n      <td valign="top" style="text-align: center">{Symbol(n)}</td> \n      <td>credit history and credit scores</td>\n      </tr></tbody></table>');
        
        formatted = this.formatPrivacySection(formatted, 'How\\?', 'All financial companies need to share customers\' personal information to run their everyday business. In the section below, we list the reasons financial companies can share their customers\' personal information; the reasons {[plsMatrix.CompanyShortName]} chooses to share; and whether you can limit this sharing.');
        
        // 3. Format main sharing table (will be inserted later after all sections)
        // formatted = this.formatPrivacySharingTable(formatted);
        
        // 4. Format "To limit our sharing" section
        formatted = this.formatPrivacyLimitSection(formatted);
        
        // 5. Format "Questions?" section
        formatted = this.formatPrivacyQuestionsSection(formatted);
        
        // 6. Format mail-in form
        formatted = this.formatPrivacyMailInForm(formatted);
        
        // 7. Format "Who we are" section
        formatted = this.formatPrivacyWhoWeAreSection(formatted);
        
        // 8. Insert main sharing table after the main FACTS table
        formatted = this.formatPrivacySharingTable(formatted);
        
        return formatted;
    }
    
    static formatPrivacySection(text, sectionName, content) {
        const pattern = new RegExp(`${sectionName}[^<]*`, 'gi');
        return text.replace(pattern, `<tr>\n  <td valign="top" style="padding: 10px; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(0, 0, 0, 1); font-size: 14pt"><b style="font-size: 14pt">${sectionName}</b></td> \n  <td style="padding: 10px; border: 1px solid rgba(188, 190, 192, 1)">${content}</td>\n  </tr><tr>\n  <td style="background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(255, 255, 255, 1); font-size: 6pt"><br></td>\n  <td></td>\n  </tr>`);
    }
    
    static formatPrivacySharingTable(text) {
        // Create the main sharing reasons table
        const sharingTable = `<div style="font-size: 6pt"><br></div>
<table width="100%" style="border-collapse: collapse"><tbody><tr>
  <td width="46%" style="padding-left: 10px; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(255, 255, 255, 1)"><b style="color: rgba(255, 255, 255, 1)">Reasons we can share your personal information</b></td> 
  <td width="27%" style="text-align: center; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(255, 255, 255, 1)"><b style="text-align: center; color: rgba(255, 255, 255, 1)">Does {[plsMatrix.CompanyLongName]} share?</b></td> 
  <td width="27%" style="text-align: center; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(255, 255, 255, 1)"><b style="text-align: center; color: rgba(255, 255, 255, 1)">Can you limit this sharing?</b></td>
  ...
  <td style="text-align: center; border: 1px solid rgba(188, 190, 192, 1)">We don't share</td>
        ...
        // Insert the sharing table after the main FACTS table (look for the first table with FACTS)
        const factsTableEnd = text.indexOf('</tbody></table>');
        if (factsTableEnd !== -1) {
            return text.substring(0, factsTableEnd + 14) + '\n' + sharingTable + text.substring(factsTableEnd + 14);
        }
        return text;
    }
    
    static formatPrivacyLimitSection(text) {
        const limitSection = `<div style="font-size: 5pt"><br></div>
<table width="100%" style="border-collapse: collapse"><tbody><tr>
  <td width="17.5%" valign="top" style="padding: 10px; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 1); border: 1px solid rgba(255, 255, 255, 1); font-size: 14pt"><b style="font-size: 14pt">To limit our sharing</b></td>
  <td style="padding: 10px; border: 1px solid rgba(188, 190, 192, 1)">
    <table width="100%"><tbody><tr>
  ...
  <td style="padding: 10px; border: 1px solid rgba(188, 190, 192, 1)">Call {[plsMatrix.CSPhoneNumber]} or go to {[plsMatrix.WebSite]}</td>
    ...
    static formatPrivacyQuestionsSection(text) {
        // This is handled in formatPrivacyLimitSection
        return text;
    }
    
    static formatPrivacyMailInForm(text) {
        const mailInForm = `  
...
<table width="100%"><tbody><tr>
  ...
  <td style="font-size: 11.5pt"><b style="font-size: 11.5pt">{Compress({[plsMatrix.CompanyLongName]}|Attn: Customer Service Department|{[plsMatrix.CompanyReturnAddr1]}|{[plsMatrix.CompanyReturnAddr2]}|{[plsMatrix.CompanyReturnAddr3]})}</b></td>
    ...
    static formatPrivacyWhoWeAreSection(text) {
...
<table width="100%" style="border-collapse: collapse"><tbody><tr>
  ...
  <td style="border: 1px solid rgba(188, 190, 192, 1)"><br></td>
    ...
    static formatPrivacySymbols(text) {
        let formatted = text;
        
        // Replace bullet points with Symbol(n)
        formatted = formatted.replace(/•/g, '{Symbol(n)}');
        formatted = formatted.replace(/[-*]\s/g, '{Symbol(n)} ');
        
        // Replace checkboxes with Symbol(q)
        formatted = formatted.replace(/☐/g, '{Symbol(q)}');
        formatted = formatted.replace(/\[\s*\]/g, '{Symbol(q)}');
        
        return formatted;
    }
    
    static formatPrivacyStyling(text) {
        let formatted = text;
        
        // Ensure proper font specification at the beginning
        if (!formatted.includes('{Font(Arial|9pt)}')) {
            formatted = '{Font(Arial|9pt)}\n' + formatted;
        }
        
        // Fix any missing closing tags - but avoid duplicating tbody
        formatted = formatted.replace(/<table([^>]*)>(?!<tbody>)/g, '<table$1><tbody>');
        formatted = formatted.replace(/<\/table>/g, '</tbody></table>');
        
        // Remove duplicate tbody tags
        formatted = formatted.replace(/<tbody><tbody>/g, '<tbody>');
        formatted = formatted.replace(/<\/tbody><\/tbody>/g, '</tbody>');
        
        return formatted;
    }
    
    static formatPrivacySpacing(text) {
        let formatted = text;
        
        // Add proper spacing between major sections
        formatted = formatted.replace(/(<\/tbody><\/table>)/g, '$1\n<div style="font-size: 6pt"><br></div>');
        
        // Add proper spacing before mail-in form
        formatted = formatted.replace(/(<hr>)/g, '$1\n\n\n\n');
...
<div>{[L003]}/{[L005]}</div>`;
    ...
    static formatUniversalDocument(text) {
        ...
        // 1. FIRST - Convert HTML from mammoth.js to our expected format
        text = this.convertHtmlToFormat(text);
        
        // 2. Convert all field names and company fields for ANY document
        text = this.fixFieldNames(text);
        
        // 3. Create proper header structure for ANY document
        text = this.createUniversalHeaderStructure(text);
        
        // 4. Create RE: table structure for ANY document
        text = this.createUniversalReTable(text);
        
        // 5. Handle special header conditions
        text = this.handleSpecialHeaders(text);
        
        // 3. Split into paragraphs - handle text that might be split incorrectly
        ...
        // 5. Apply universal formatting rules to the joined text
        finalText = this.handleUniversalTranslationParagraphs(finalText);
        finalText = this.addUniversalSalutations(finalText);
        finalText = this.wrapUniversalMoneyFields(finalText);
        finalText = this.createUniversalMaturityTables(finalText);
        finalText = this.applyUniversalBoldFormatting(finalText);
        finalText = this.fixUniversalTableStructure(finalText);
        finalText = this.fixUniversalMathFunctionSpacing(finalText);
        
        // 6. Handle compression functions across the entire document (after all paragraphs are processed)
        ...
        // 18. Field names already converted at the beginning
...
<div>TOTAL YOU MUST PAY TO CURE DEFAULT: {Math({[C001]} + {[M585]} - {[M013]}|Money)}</div>`;
    ...
    static addMissingHeaders(formatted) {
        // Universal rule: Format headers that are already present in the document
        // Only format headers that exist in the document, don't replace with hardcoded text
        
        // Format any header text that appears to be a title/header (contains "Notice" and is not already formatted)
        formatted = formatted.replace(
            /<div>(Notice of [^<]+)<\/div>/g,
            (match, headerText) => {
                // Only format if it's not already formatted as a header
                if (!match.includes('text-align: center') && !match.includes('<b>')) {
                    return `<div style="text-align: center"><b>${headerText}</b></div>`;
                }
                return match;
            }
        );
    ...
    static fixFieldNames(formatted) {
        // Remove E6/E7/E8 suffixes from specific fields
        ...
        // Handle L001E8 -> L001 conversion (critical for SL106) - handle both raw and bracketed formats
        formatted = formatted.replace(/#L001E8#/g, '{[L001]}');
        formatted = formatted.replace(/\{\[?L001E8\]?\}/g, '{[L001]}');
        formatted = formatted.replace(/#L001E7#/g, '{[L001]}');
        formatted = formatted.replace(/\{\[?L001E7\]?\}/g, '{[L001]}');
        
        // Handle other field name conversions for SL106 and other documents
        formatted = formatted.replace(/#M558#/g, '{[M558]}');
        formatted = formatted.replace(/#M559#/g, '{[M559]}');
        formatted = formatted.replace(/#M560#/g, '{[M560]}');
        formatted = formatted.replace(/#M561#/g, '{[M561]}');
        ...
        // Handle other E6/E8 suffix removals
        ...
        // Convert company fields from <FieldName> to {[plsMatrix.FieldName]} format
        formatted = formatted.replace(/<CompanyLongName>/g, '{[plsMatrix.CompanyLongName]}');
        formatted = formatted.replace(/<CSPhoneNumber>/g, '{[plsMatrix.CSPhoneNumber]}');
        formatted = formatted.replace(/<LoanCounselingPh>/g, '{[plsMatrix.LoanCounselingPh]}');
        formatted = formatted.replace(/<CompanyReturnAddr1>/g, '{[plsMatrix.CompanyReturnAddr1]}');
 ...
 <br>
  ...
  <td>${bullet.trim()}</td>
    ...
    static applyDocumentSpecificFormatting(formatted) {
        ...
        // Handle Privacy Policy documents
        if (formatted.includes('Privacy Policy') || formatted.includes('FACTS') || formatted.includes('Federal Privacy Model Form')) {
            console.log('Applying Privacy Policy specific formatting...');
            
            // Apply privacy-specific formatting
            formatted = this.formatPrivacyDocument(formatted);
        }
        
        // Handle SL106 documents (maturity date letters)
        if (formatted.includes('L001E8') && formatted.includes('maturity date') && formatted.includes('payoff statement')) {
            console.log('Applying SL106 specific formatting...');
            
            // Apply SL106-specific formatting
            formatted = this.formatSL106Document(formatted);
        }
    ...
    // ===== UNIVERSAL FORMATTING METHODS FOR ANY DOCUMENT =====
    
    static createUniversalHeaderStructure(text) {
        // Universal rule: Detect L001 + M558-M566 sequence and convert to proper header structure
        // Pattern: {[L001]} followed by {[M558]} through {[M566]} should become {[tagHeader]} + right-aligned {[L001]} + {[mailingAddress]}
        
        const headerPattern = /<div>\{\[L001\]\}<\/div>\s*<br>\s*<div>\{\[M558\]\}<\/div>\s*<br>\s*<div>\{\[M559\]\}<\/div>\s*<br>\s*<div>\{\[M560\]\}<\/div>\s*<br>\s*<div>\{\[M561\]\}<\/div>\s*<br>\s*<div>\{\[M562\]\}<\/div>\s*<br>\s*<div>\{\[M563\]\}, \{\[M564\]\} \{\[M565\]\} \{\[M566\]\}<\/div>/g;
    ...
    static createUniversalReTable(text) {
        // Universal rule: Detect "Re: Loan No:" + "Property Address:" pattern and convert to proper table structure
        
        const reTablePattern = /<div>Re: Loan No: \{\[M594\]\}<\/div>\s*<br>\s*<div>Property Address: \{\[M567\]\}<\/div>\s*<br>\s*<div>\{\[M568\]\}<\/div>/g;
        
        if (reTablePattern.test(text)) {
            text = text.replace(reTablePattern,
                '<div><table width="100%" style="border-collapse: collapse"><tbody><tr>\n  <td width="20%">RE: Loan No:</td>\n  <td>{[M594]}</td>\n  </tr><tr>\n  <td width="20%" valign="top">Property Address:</td>\n  <td>{Compress({[M567]}|{[M583]}|{[M568]})}</td>\n</tr></tbody></table></div>'
            );
        }
    ...
    static handleUniversalTranslationParagraphs(text) {
        // Universal rule: Detect translation assistance paragraphs and center-align them
        
        const translationPattern = /<div>For homeowners who require any translation assistance or Language Access Services, please contact us at \{\[plsMatrix\.CSPhoneNumber\]\}\. A translation of this letter into a language other than English may be obtained\.<\/div>\s*<br>\s*<div>Si necesita asistencia con la traduccion o servicios de acceso al idioma, llamenos al \{\[plsMatrix\.CSPhoneNumber\]\}\.<\/div>\s*<br>\s*<div>Se puede obtener una traduccion de esta carta\.<\/div>/g;
        
        if (translationPattern.test(text)) {
            text = text.replace(translationPattern,
                '<div style="text-align: center">For homeowners who require any translation assistance or Language Access Services, please contact us at {[plsMatrix.CSPhoneNumber]}. A translation of this letter into a language other than English may be obtained.</div><div style="text-align: center">Si necesita asistencia con la traduccion o servicios de acceso al idioma, llamenos al {[plsMatrix.CSPhoneNumber]}.</div><div style="text-align: center">Se puede obtener una traduccion de esta carta.</div>'
            );
        }
    ...
    static addUniversalSalutations(text) {
        // Universal rule: Add "Dear Mortgager(s)," after translation paragraphs if not present
        
        const salutationPattern = /<div style="text-align: center">Se puede obtener una traduccion de esta carta\.<\/div>\s*<br>\s*<br>\s*<div>This notice is to inform you/g;
        
        if (salutationPattern.test(text)) {
            text = text.replace(salutationPattern,
                '<div style="text-align: center">Se puede obtener una traduccion de esta carta.</div>\n<br>\n<br>\n<div>Dear Mortgager(s),</div>\n<br>\n<div>This notice is to inform you'
            );
        }
    ...
    static wrapUniversalMoneyFields(text) {
        // Universal rule: Wrap U234E4 fields in Money() function
        
        text = text.replace(/\$\{\[U234E4\]\}/g, '{Money({[U234E4]})}');
        text = text.replace(/\{\[U234E4\]\}/g, '{Money({[U234E4]})}');
        
        return text;
    }
    
    static createUniversalMaturityTables(text) {
        // Universal rule: Convert maturity date info to table format
        
        const maturityPattern = /<div>Maturity date:<\/div>\s*<br>\s*<div>\{\[U072\]\}<\/div>\s*<br>\s*<div>Non-Interest Balance:<\/div>\s*<br>\s*<div>\{Money\(\{\[U234E4\]\}\)\}<\/div>/g;
        
        if (maturityPattern.test(text)) {
            text = text.replace(maturityPattern,
                '<div><table width="100%" style="border-collapse: collapse"><tbody><tr>\n  <td width="50%">Maturity date:</td>\n  <td>{[U072]}</td>\n  </tr><tr>\n  <td width="50%">Non-Interest Balance:</td>\n  <td>{Money({[U234E4]})}</td>\n</tr></tbody></table></div>'
            );
        }
    ...
    static applyUniversalBoldFormatting(text) {
        // Universal rule: Fix bold formatting issues that come from Word document processing
        // Only fix structural issues, don't add arbitrary bold formatting
        
        // Fix incomplete bold tags that might have been split during processing
        text = text.replace(/<div><b>([^<]+)<\/div>/g, '<div><b>$1</b></div>');
        
        // Fix bold tags that are missing closing tags
        text = text.replace(/<b>([^<]+)<\/div>/g, '<b>$1</b></div>');
        
        // Fix bold tags that are missing opening tags
        text = text.replace(/<div>([^<]+)<\/b>/g, '<div><b>$1</b></div>');
        
        return text;
    }
    
    static fixUniversalTableStructure(text) {
        // Universal rule: Fix table structure issues for ANY document
        // Add missing closing div tags where they logically belong
        
        // Fix missing closing div tag after any table that's followed by a br and div
        text = text.replace(
            /<\/tr><\/tbody><\/table>\s*<br>\s*<div/g,
            '</tr></tbody></table></div>\n<br>\n<div'
        );
        
        // Fix missing closing div tag after any table that's followed by a br and div with style
        text = text.replace(
            /<\/tr><\/tbody><\/table>\s*<br>\s*<div style=/g,
            '</tr></tbody></table></div>\n<br>\n<div style='
        );
    ...
    static fixUniversalMathFunctionSpacing(text) {
        // Universal rule: Fix Math function spacing issues for ANY document
        
        // Fix Math function split across lines
        text = text.replace(
            /<div>As of \{\[L001\]\}, your loan is delinquent and due for \{\[M026\]\}, in the past due amount of<\/div>\s*<br>\s*<div>\{Math\(\{\[C001\]\} \+ \{\[M585\]\} - \{\[M013\]\}\|Money\)\}\.<\/div>/g,
            '<div>As of {[L001]}, your loan is delinquent and due for {[M026]}, in the past due amount of</div>\n<div>{Math({[C001]} + {[M585]} - {[M013]}|Money)}.</div>'
        );
        
        return text;
    }
    
    static convertHtmlToFormat(text) {
        // Universal rule: Convert mammoth.js HTML output to our expected format
        // Convert <p> tags to <div> tags and preserve bold formatting
        ...
        // Convert <em> tags to <i> tags for consistency
        text = text.replace(/<em>/g, '<i>');
        text = text.replace(/<\/em>/g, '</i>');
        
        // Handle line breaks - mammoth.js might use <br> tags
        text = text.replace(/<br\s*\/?>/g, '\n');
        
        // Remove any remaining HTML tags we don't want
