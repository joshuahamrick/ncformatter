#!/usr/bin/env python3
"""Extract content from LM207 document"""
import sys
import os
...
doc_path = os.path.join(os.path.dirname(__file__), '..', 'LM207 - Short Sale Counter Offer - Keesler - V1.0.docx')
...
for i, block in enumerate(ir.get('blocks', [])):
    if block.get('type') == 'paragraph':
        ...
        if text:
            ...
            try:
                print(f"{i+1:3d}: {text[:400]}{fmt_str}")
            except UnicodeEncodeError:
                print(f"{i+1:3d}: {text[:400].encode('ascii', 'ignore').decode('ascii')}{fmt_str}")
